...
static DWORD64 ms_OnlineVehicleDespawnPatchPtr = 0;
	...
	if (!strcmp(thread->GetName(), "shop_controller"))
	{
		if (!ms_RanOnlineVehicleDespawnPatch)
		{
			...
			if (program->m_CodeBlocks)
			{
				...
				{
					ms_OnlineVehicleDespawnPatchPtr = handle.Addr() + 12;
					memcpy_s(ms_OnlineVehicleDespawnPatchOrigBytes.data(), ms_OnlineVehicleDespawnPatchOrigBytes.size(),
					         reinterpret_cast<void *>(ms_OnlineVehicleDespawnPatchPtr), 3);

					Memory::Write<BYTE>(reinterpret_cast<BYTE *>(ms_OnlineVehicleDespawnPatchPtr), 0, 3);
...
{
	if (ms_OnlineVehicleDespawnPatchPtr)
	{
		memcpy_s(reinterpret_cast<void *>(ms_OnlineVehicleDespawnPatchPtr), 3,
